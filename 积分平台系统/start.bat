...
REM ============================================================
REM  Points Platform startup script
REM  - Detect Python
...
for /f "tokens=2" %%i in ('%BASE_PY% --version 2^>^&1') do set "PY_VERSION=%%i"
...
if exist "%VENV_DIR%\Scripts\python.exe" (
    ...
    if errorlevel 1 (
        echo [2/4] stale venv detected, rebuilding .venv ...
        rmdir /s /q "%VENV_DIR%"
    )
)
...
start "PointsPlatformServer" /MIN "%PY_EXE%" app.py >nul 2>&1
